...
<c:import url="/common/base.jsp">
	...
	<c:param name="content">
		...
		<form action="ClassUpdateExecute.action" method="post" class="container mt-4" style="max-width: 500px;">
			...
			<input type="hidden" name="classnum" value="<c:out value="${classNum.class_num}"></c:out>">

			<%-- クラス名 --%>
			<div class="mb-3">
				<label class="form-label fw-bold">クラス番号</label>
				<input type="text" name="newclassnum" class="form-control"
					value="<c:out value="${classNum.class_num}"></c:out>"
					placeholder="クラス番号を入力してください" maxlength="5" required>
				<div class="text-danger small">${errors.num}</div>
			</div>
